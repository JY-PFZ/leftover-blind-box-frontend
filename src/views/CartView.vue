           ...
           >
             ...
             <div class="item-info">
               ...
               <p v-if="item.magicbagId" class="item-merchant">
                 ...
                 <!-- 🟡 Debug 输出 -->
                 <!-- <span style="color: blue; font-size: 0.8em; display: block;">(Debug value: {{ item.magicbagId }} | Type: {{ typeof item.magicbagId }})</span> -->
               </p>
               <!-- 🟡 显示 invalid 警告 -->
 ...
 function checkout() {
   ...
   if (!cart.items || cart.items.length === 0) { // 检查 items 是否存在且不为空
     ...
     // 可以添加一个用户提示，例如使用简单的 alert 或更好的 UI 提示
 ...
 async function processPayment() {
   ...
   try {
     ...
     await new Promise(resolve => setTimeout(resolve, 1000)); // 缩短延迟
